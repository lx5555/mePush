...
public class RedisUtils {
    ...
    public Map<String, String> mGet(List<String> keys) {
        ...
        try {
            List<String> value = redisTemplate.opsForValue().multiGet(keys);//批量获取
    ...
    public List<String> lRange(String key, long start, long end) {
        try {
            return redisTemplate.opsForList().range(key, start, end);//查询以key为list名字的范围数据
        } catch (Exception e) {
    ...
    public void pipelineSetEx(Map<String, String> keyValues, Long seconds) {
        try {
            redisTemplate.executePipelined((RedisCallback<String>) connection -> {
                for (Map.Entry<String, String> entry : keyValues.entrySet()) { //setEx设置key-value并且原子性设置过期时间
    ...
    public void lPush(String key, String value, Long seconds) {
        try {
            redisTemplate.executePipelined((RedisCallback<String>) connection -> {
                ...
                connection.expire(key.getBytes(), seconds); //expire指定 过期时间
                return null;
            });
        } catch (Exception e) {
            log.error("RedisUtils#pipelineSetEx fail! e:{}", Throwables.getStackTraceAsString(e));
    ...
    public Boolean execLimitLua(RedisScript<Long> redisScript, List<String> keys, String... args) {
        ...
        try {
            Long execute = redisTemplate.execute(redisScript, keys, args); //redis lua脚本
